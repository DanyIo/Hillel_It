...
function greatestcommonden(x = "Введіть хоча б 1 число.", y) {
  if (x < 0 || y < 0) return "Одне з введених чисел меньше за 0.";
  if (y > x) return greatestcommonden(y, x);
...
function perfectnumber(number) {
  if (number == undefined) {
    ...
    return number === k ? "Число є досконалим." : "Число не є досконал.";
  }
}
...
function sumofarray(num1, num2) {
  if (num1 === undefined || num2 === undefined) {
    ...
    if (num1 > num2) return sumofarray(num2, num1);
    sum = 0;
